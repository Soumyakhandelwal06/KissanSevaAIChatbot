...
// --- Feature Lists based on your model description ---
...
// ===================================================================================
// NEW: Landing Page Component (No Change)
// ===================================================================================
const LandingPage = ({ onEnterChat }) => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-300 via-sky-400 to-cyan-500 text-white flex flex-col items-center justify-center p-4 overflow-hidden">
...
const FarmerChatbot = () => {
  ...
  // --- UPDATED: State for basic context and complex features ---
  const [context, setContext] = useState({
    crop: "rice",
    location: "Kerala",
    season: "kharif" 
  ...
  // NEW: State for the 20 features required by Prediction Models
  const initialFeatureState = {
    // Numerical
    Temparature: 25, Humidity: 60, Moisture: 30, Nitrogen: 80, Potassium: 50, Phosphorous: 60,
    // Categorical
    Soil_Type: "Clayey", 
    Crop_Type: "Paddy" 
  };
  const [predictionFeatures, setPredictionFeatures] = useState(initialFeatureState);
  // --- END State Update ---
  ...
  // ===============================================================================
  // UPDATED: sendTextMessage (FastAPI /api/chat endpoint)
  // Logic updated to conditionally include 'features' for prediction requests.
  // ===============================================================================
  const sendTextMessage = async (messageText = null, isPrediction = false) => {
    const message = messageText || textInput.trim();
    if (!message && !isPrediction) return; // Allow empty message if it's a prediction request

    const userQuery = isPrediction ? `Run prediction for ${predictionFeatures.Crop_Type} with NPK: ${predictionFeatures.Nitrogen}/${predictionFeatures.Phosphorous}/${predictionFeatures.Potassium}` : message;

    // Add user message to chat
    addMessage(userQuery, true, "text");
    setTextInput("");
    setIsLoading(true);
    ...
    if (isPrediction) {
        // Embed the full 20-feature object for the prediction model
        requestContext.features = predictionFeatures; 
    }
    
    const requestBody = {
      query: message || userQuery, // Send the original message or the synthetic prediction query
    ...
    try {
      ...
      if (response.ok) {
        // Response matches ChatResponse model
        const confidenceDisplay = (data.confidence * 100).toFixed(1);
        let answerContent = `Answer: ${data.answer}\n\n`;
        answerContent += `Confidence: ${confidenceDisplay}% (Intent: ${data.intent})\n`;
        answerContent += `Model: ${data.used_model}`;
  ...
  // ===============================================================================
  // UPDATED: uploadImage (No functional change)
  // ===============================================================================
  const uploadImage = async () => {
    const file = imageInputRef.current?.files[0];
    if (!file) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-950 to-purple-950 p-4 text-white">
      <div className="max-w-10/12 mx-auto">
        ...
        <div className="bg-gray-800/95 backdrop-blur-md shadow-2xl rounded-b-3xl overflow-hidden">
          {/* Tab Navigation (UPDATED to include 'features') */}
          <div className="bg-gray-900 px-6 pt-6">
            <div className="flex gap-2">
              <TabButton
                id="text"
